...
## 2.4 Archive teasers — why the minter writes them

The archive lists ~150 boards per sport, and its cards were first titled "Career path #7": a
filing label that tells a browsing player nothing. They now carry a **teaser** — a real
low-reveal fact about the subject plus a jab about the shape of their career:

> Part of the 2003 draft class — and no forwarding address.
> Broke in during the 1960s and last played in 1978 — with one very familiar reunion.

**This has to be generated by the minter, not templated on the client, and the reason is
structural.** The app deliberately never sees the answer, so a client-side teaser can only ever
joke about the *shape* of a path — 525 boards sharing ~20 lines. The minter has the subject's
actual facts, and it already owns a library for phrasing them: `whoami_clues.DIMENSIONS`, the
same ~30 pronoun-free builders the Who Am I? clue engine draws from, each carrying a `reveal`
score. Two filters make a clue safe as a teaser:

- `reveal <= MAX_TEASER_REVEAL` (0.36) — a teaser hints, it doesn't solve. Nickname, career stat
  line, best season and draft slot are exactly the dimensions that *would* solve it, and they all
  sit above the cut.
- not the `team` family — the board already *is* the club history, so "Suited up for 6 different
  franchises" is the card's own subtitle wearing a hat.

Then three rules the live pool taught, each with a test:
- **The jab may not echo a word from the fact.** First draft: "Put together an 8-season career —
  with a tidy little career to show for it."
- **Only a two-club career may say "one move".** It was landing on three-club careers.
- **The line has to fit the card** (`MAX_TEASER_CHARS` = 72), preferring the *longest* fact that
  fits so the budget buys information rather than collapsing every card to "Pitcher — …". The
  first draft's longest was 96 characters: four lines of card for one gag.

Result: 470 distinct lines across 525 boards, deterministic per subject (a card must not reword
itself when the pool is regenerated), and leak-checked against the answer at upsert by the same
word-boundary rule the clue text uses.

`JourneymanTeaser.swift` survives as the **fallback** for content with no minted teaser — the
optional field means the client must still title a card, and the shape-only joke is what it can
honestly manage without knowing the player.
