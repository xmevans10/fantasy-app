...
| **M18** | Draft & Spin: per-round mechanic + real full-roster data coverage | [M18-draft-spin-rosters-and-coverage.md](M18-draft-spin-rosters-and-coverage.md) | ✅ shipped 2026-07-09: mechanic re-verified live post era→year swap (215 tests + all-5-sport board/result screenshots), **NBA full-league rosters 2002+ via a new hoopR provider** (season rows 8,199→13,152, players 855→2,511, median team-year ~13–17 deep — live-verified before/after), MLB leader sweep widened 1955+ (pool 3,298→4,362), NFL verified uniformly deep 1999–2024, soccer/tennis ceiling documented as final in BALLIQ_SPEC §3. **Same-day follow-up (user-directed):** per-game setup screens for every format (Home sport chips removed), Draft & Spin setup rows (Teams one-team lock ・ Season Variations prime-only ・ NFL Roster locked to offense), casino-grade spin reveal, truly-random spins (RNG-injected), scoring-audit fix (lineup power now drives the record via `winProbability`), MLB sweep widened to 1901 (pool 7,109 ids), and tennis's first real bulk source (`tennis_atp.py`, Sackmann snapshot 1968–2018 with Wikipedia-verified headshots) — see spec §8 M18 follow-up notes |
