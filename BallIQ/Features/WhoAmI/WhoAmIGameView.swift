...
struct WhoAmIGameView: View {
    ...
    /// See `WhoAmILockedClueRow`. Inside a blitz the price is a share of the board's own
    /// maximum, never points — same rule as the header and the Next clue button
    /// (`BlitzBoardValue`).
    private func lockedRow(position: Int) -> some View {
        let cost = WhoAmIScoring.cost(toUnlock: position, difficulty: puzzle.difficulty)
        return WhoAmILockedClueRow(
            position: position,
            costText: blitz == nil
                ? String(localized: "−\(cost) pts")
                : BlitzBoardValue.cost(cost, of: WhoAmIScoring.maxScore(difficulty: puzzle.difficulty)),
            accessibilityText: blitz == nil
