...
**Puzzle Blitz (M27)** is listed separately because it is **not a format** — it is a fixed-length
run (1/3/5 min) that serves whole, real boards of K4C4 / Who Am I? / Journeyman / Over/Under back
to back from the sports and formats you pick, and shows a score only once the clock stops. It
reuses each format's own game view through a `BlitzSession` seam, so it inherits their boards
rather than copying them. See §8's M27 row for the scoring purse and what's deliberately left out.
...
| M24 ladder retuned on score | ⚠️ **code shipped, content NOT reseeded** — `bot_skill` is now solved against what the bot *scores* rather than how often a reference player beats it, and the clock left the ladder's comparable. Held back deliberately: the new curve has a flat bottom (rungs 1–6 pin at minimum skill, 99–100% player win) and that is the tier a new player meets first. **Open decision: the Bronze shape** — fewer Bronze rungs, or a board curve starting nearer the 0.18 floor. Also corrects a wrong claim made in-session: 2/8 IS reachable on Keep4; the real constraint is that you cannot have both an easy board and a bad bot. See `prompts/M24-ladder-retune.md`. **Superseded by M32 / v1.8.7 (2026-09-21):** the ladder left the score objective and per-format boards for Blitz win-rate solving, so the open Bronze-shape decision is moot — one curve now descends through every format. |
...
| M32 bot knowledge + Blitz ladder (v1.8.7) | ✅ submitted for review 2026-09-21 (build 55) — the Ladder stops being single boards and becomes **Puzzle Blitz runs**, moving `LadderRung.mode` off `PuzzleFormat` onto a lenient-decoding `LadderMode` (`blitz4k`) so a server-side mode can never empty the tab. Every per-format rung was fighting a floor its format could not get under and the two floors pointed opposite ways (keep4's forced 4/4 split floors the win rate at ~0.27, ~0.63 on an easy board — which is why rungs 1–9 shipped at minimum skill and were won 99.8% of the time; Who Am I?'s clue ladder saturates so a perfect bot still lost ~75%). A blitz run rebases chance to exactly zero (`BlitzScoring.surplus`) and aggregates several boards, so the curve is solvable again: measured **1.000 → 0.011** across the skill range, and the seeder solves for win rate directly. Rungs are now **seeded duels** — one board sequence per attempt, the same boards for both sides, points decide (ties to the player, no speed term). **Bots get knowledge:** an era window, per-sport deltas and a fame bias, mirrored in `ladder.py` and pinned in both languages, so Solomon is genuinely unbeatable on the forgotten decades and Nova is the identity profile. Migrations 0029/0030 applied live, 30 blitz rungs seeded, `bots.knowledge` populated. **The live DB is deliberately ahead of the shipped client and that is safe by construction:** a build that predates `LadderMode` decodes `mode='blitz'` as `.keep4` through `PuzzleFormat`'s existing unknown-case fallback, so the tab degrades to the old per-format pool rather than emptying; build 55 closes the gap. 847 Python tests green; Swift suite not re-run this session (simulators avoided by request). |

**Release status (updated 2026-09-21):** **1.8.7 (build 55) is `WAITING_FOR_REVIEW`**, submitted
2026-09-21 16:38 UTC, carrying the M32 bot-knowledge + Blitz-ladder work. **1.8.6 (build 54) is
the current `READY_FOR_SALE` version** (Week Packs lead Home until finished, for at most two days).
The live ASC API confirms the whole 1.8.x line is `READY_FOR_SALE` back to 1.8.0; the earlier
1.7.0 note below is retained only as history.

**Prior release status (2026-08-25):** **1.7.0 (build 40) was `WAITING_FOR_REVIEW`** as of
16:50 UTC, carrying M23 live duels, M25 no-timers and M27 Puzzle Blitz together.

It shipped a day late for a mechanical reason worth remembering: the 1.7.0 `appStoreVersion` was
cut on 2026-08-24 while the pbxproj still read `MARKETING_VERSION = 1.6.0`, so build 39 uploaded
`CFBundleShortVersionString` 1.6.0 against a 1.7.0 record. Apple requires the two to match — the
version went `INVALID_BINARY`, its review-submission item went `REJECTED`, and the live-duel and
no-timer work sat unreleased. Fixed by bumping both `MARKETING_VERSION` (→1.7.0) and
`CURRENT_PROJECT_VERSION` (→40) and re-uploading; the version then flipped to
`PREPARE_FOR_SUBMISSION` on attach and was resubmitted **through the existing
`UNRESOLVED_ISSUES` submission** (the rejection path — not a new `POST`). Two diagnostic traps
cost time and are now written up in `.claude/skills/testflight-release/SKILL.md`: an invalid
`fields[...]` list returns an empty array rather than a 400 (which reads as "no open
submissions"), and the submit `PATCH`'s propagation lag outlasted two retries.

Also caught by the archive and not by any amount of simulator work: `DebugLaunch` keeps parallel
`#if DEBUG` / `#else` declarations, and M27's two new flags were added only to the DEBUG half, so
the Release build failed with "type 'DebugLaunch' has no member". **Adding a `DebugLaunch` flag
means editing both halves.**

**Prior release status (2026-08-24):** 🎉 **1.6.0 shipped AND monetization finally works.**
