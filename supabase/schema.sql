...
create table if not exists public.puzzles (
  id          text primary key,
  sport       text not null,                 -- 'nfl' | 'nba' | 'baseball' | 'soccer'
                                            -- | 'tennis' | 'hockey' | 'f1'
...
-- athlete_id, NHL playerId, Transfermarkt player_id). Null for the sweeps that carry none.
--
-- `id` is name-keyed (`nfl-david-johnson-2016`) and therefore cannot tell two same-sport
-- players with one name apart. That is not theoretical: the 2026-08-29 Journeyman daily
-- welded NFL TE David Johnson (2009-2016) onto RB David Johnson (2015-2022) and shipped a
-- seven-club career path neither man had. Every pool that folds seasons into a career now
-- groups on this column instead of on `name` — see tools/ingest/whoami_pool.person_key.
alter table public.player_seasons add column if not exists person_id  text;
-- Serves the career/season person join the pool builders do per sport.
create index if not exists player_seasons_sport_person_idx
  on public.player_seasons (sport, person_id) where person_id is not null;
-- Serves `upsert.fetch_catalog_ids_missing(sport, 'person_id')`, which pages
-- `sport=eq.X&person_id=is.null&order=id` to find the rows a run can still fill in. Without
-- it that scan walks the whole sport partition and hits the 57014 statement timeout — which
-- is what stopped the first person_id backfill dead after 1000 ids. Partial on IS NULL, so it
-- shrinks toward empty as the column fills: the mirror of the index above, not a duplicate.
create index if not exists player_seasons_missing_person_idx
  on public.player_seasons (sport, id) where person_id is null;
